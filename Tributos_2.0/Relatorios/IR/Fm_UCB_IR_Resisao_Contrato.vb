...
Public Class Fm_UCB_IR_Resisao_Contrato
    Private Sub Fm_UCB_IR_Resisao_Contrato_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Preencher_Data()
        Try
            sql = "Select DataRecolhimento from IR_Multa_Rescisao where Fechado="
            sql += "'" & "S" & "'"
            sql += "" & "ORDER BY DataRecolhimento DESC" & ""
            conectar.Open()
            adaptar = New SqlDataAdapter(sql, conectar)
            conectar.Close()
            local = New DataSet
            ...
            Cb_Ano.ValueMember = "Id"
            Cb_Ano.DisplayMember = "DataRecolhimento"
            Cb_Ano.DataSource = local.Tables(0)
            Cb_Ano.SelectedIndex = 0
        Catch ex As Exception
