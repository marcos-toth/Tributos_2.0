...
Public Class Fm_cadastro_IR_Recisao_Aluguel
    Private Sub Fm_cadastro_IR_Recisao_Aluguel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub txtValor_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtValor.KeyPress
        ...
        End If
    End Sub
    Private Sub Txt_reducao_KeyPress(sender As Object, e As KeyPressEventArgs) Handles Txt_vBase.KeyPress
        If e.KeyChar = "." Then
            e.KeyChar = ","
        ElseIf Not Char.IsNumber(e.KeyChar) And Not e.KeyChar = vbBack And Not e.KeyChar = "," Then
            e.Handled = True
    ...
    Private Sub ValidaCadastro()
        Try
            ...
            End If
            sql = "select * from IR_Multa_Rescisao where CPF_CNPJ="
            ...
            conectar.Close()
            local = New DataSet
            adaptar.Fill(local, "IR_Multa_Rescisao")
            resultado = local.Tables("IR_Multa_Rescisao").Rows.Count

            If resultado > 0 Then
                registro = local.Tables("IR_Multa_Rescisao").Rows(0)
    ...
    Private Sub Carregar_IR()
        ...
        Try
            sql = "Select sum(Valor) Valor from IR_Multa_Rescisao  Where codEntidade="
    ...
    Private Sub DataGrid()
        Try
            DG_IR.Rows.Clear()
            conectar.Open()
            sql = "Select DataRecolhimento, Mes, NomeBeneficiario, CPF_CNPJ,ValorBase, Valor from IR_Multa_Rescisao where Mes="
            sql += "'" & vlMes & "'and Ano="
            sql += "'" & vlAno & "'and EntidadeLogada="
            sql += "'" & EntidadeLogada & "'and CodEntidade="
            ...
            Do While dr.Read
                DG_IR.Rows.Add(dr("DataRecolhimento"), dr("Mes"), dr("NomeBeneficiario"), dr("CPF_CNPJ"), dr("ValorBase"), dr("Valor"))
            Loop
            conectar.Close()
            'Limpar()
        Catch ex As Exception
        ...
        If Txt_Nome.Text <> "" Then
            ...
            If resultado > 0 Then
                ...
                If MsgBox("Deseja Realmente Alterar Esse Cadastro!!", MsgBoxStyle.YesNo + vbCritical, "Alterar Registro") = MsgBoxResult.Yes Then
                    sql = "update IR_Multa_Rescisao set valor='" & vValor & "' ,"
                    sql += "Data='" & txtData.Text & "' ,"
                    sql += "NomeBeneficiario='" & Txt_Nome.Text & "' ,"
                    sql += "ValorBase='" & vlBase & "' ,"

                    sql += "Mes='" & vlMes & "' ,"
                    sql += "Data_Deposito='" & Txt_Data_Deposito.Text & "' ,"
                    sql += "Usuario='" & Usuario & "' "
                    sql += " where Id='" & Id & "' "
                    Conectar_Banco()
                    'Carregar_IR()
            ...
            ElseIf resultado = 0 Then
                ...
                sql = "insert into IR_Multa_Rescisao (Data,CodEntidade,NomeEntidade,EntidadeMae,ValorBase,Valor,Usuario,Ano,Mes,NomeBeneficiario,EntidadeLogada,CPF_CNPJ,Justificativa,Data_deposito,Contabilizado,Ano_Calendario)"
    ...
    Private Sub btnDeletar_Click(sender As Object, e As EventArgs) Handles btnDeletar.Click
        If Contabilizado = "N" Then
            ...
            If MsgBox("Deseja Realmente Deletar Esse Cadastro!!", MsgBoxStyle.YesNo + vbCritical, "Deletar Registro") = MsgBoxResult.Yes Then
                If resultado > 0 Then
                    sql = "delete from IR_Multa_Rescisao where Id='" & Id & "'"
                    Conectar_Banco()
                    'Carregar_IR()
                    'vValor_Banco = Replace(vValor_Banco, ".", "")
                    'vValor_Banco = Replace(vValor_Banco, ",", ".")
        ...
        DataGrid()
    End Sub
    Private Sub Txt_Justificativa_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Txt_Justificativa.TextChanged
        Dim selStart As Integer
        Dim selLength As Integer
